    ...
    <AuthenticatedLayout>
        <template #header>
            ...
            <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 mt-4" v-for="game in allGames" :key="game.id">
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-4 flex items-start">
                    ...
                    <div class="w-1/5 mx-2 text-center">
                        ...
                        <span class="text-gray-400 p-2 rounded-lg border-[1px] border-gray-400 mb-2 block" :class="[(game.home_score == game.prediction.home_score && game.away_score == game.prediction.away_score) ? 'bg-green-100' : 'bg-red-100']" v-if="game.prediction && game.time_elapsed != 'notstarted'">
                            {{game.prediction.home_score}}-{{game.prediction.away_score}}
                        </span>
                        <div v-if="game.prediction && game.time_elapsed == 'notstarted'">
                            <div class="text-gray-400 p-2 rounded-lg border-[1px] border-gray-400 mb-2 block relative">
                                <div>{{predictionInputs[game.id+'_home_score']}}-{{predictionInputs[game.id+'_away_score']}}</div>
